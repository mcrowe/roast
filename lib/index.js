...
function assert(condition, msg) {
  if (!condition) {
    throw new Error('Roast Error: ' + msg);
  }
}

function validateDbAndTable(schema, db, table) {
  assert(_.isPlainObject(db), '"db" must be a plain object');
  assert(_.isString(table), '"table" must be a string');
  assert(schema[table], 'table does not exist \'' + table + '\'');
}

function validateWhere(where) {
  assert(_.isUndefined(where) || _.isFunction(where), '"where" must be a function, or undefined');
}

function validateId(schema, table, id) {
  var errors = columnErrors(schema[table].id, id);
  assert(_.isEmpty(errors), '"id" ' + errors[0]);
}

function validateValues(values) {
  assert(_.isPlainObject(values), '"values" must be a plain object');
}
...
Roast.createRepo = function (schema) {
  ...
  return {
    get: function get(db, table, id) {
      validateDbAndTable(schema, db, table);
      validateId(schema, table, id);
    ...
    insert: function insert(db, table, values) {
      validateDbAndTable(schema, db, table);
      validateValues(values);

      var record = applyDefaults(schema[table], db[table], values);
    ...
    insert_: function insert_(db, table, values) {
      return this.insert(db, table, values)[0];
    },
    delete_: function delete_(db, table, id) {
      return this.delete(db, table, id)[0];
    },
    update_: function update_(db, table, id, values) {
