...
function validateWhere(where) {
  assert(
    _.isUndefined(where) || _.isFunction(where),
    '"where" must be a function, or undefined'
  )
}

function validateId(schema, table, id) {
  const errors = columnErrors(schema[table].id, id)
...
Roast.createRepo = schema => {
  ...
  return {
    ...
    insert(db, table, values) {
      ...
      const record = applyDefaults(schema[table], db[table], values)
